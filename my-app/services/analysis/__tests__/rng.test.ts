/**
 * The generator is the foundation of the determinism promise, so it is pinned by
 * its actual output, not only by its properties. If a refactor changes these
 * numbers, every shared result URL in existence starts returning different
 * numbers, and that has to fail loudly here rather than quietly in production.
 */
import { describe, expect, it } from "vitest";
import { bell, clamp, mulberry32, pickDistinct, roundTo, signedBell, uniform } from "../rng";
...
describe("mulberry32", () => {
  ...
  it("is pinned to a known output for a known seed", () => {
    ...
    const drawn = [rng(), rng(), rng()].map((value) => roundTo(value, 12));
    expect(drawn).toEqual([0.60110375192, 0.448290558998, 0.85246579349]);
  });
...
describe("helpers", () => {
   ...
   * epsilon and changes every synthetic value in the service.
   *
   * 1.005 is not 1.005 in binary floating point, it is 1.00499999999999989.
   * So `Math.round(1.005 * 100)` is `Math.round(100.49999999999999)`, which is
   * 100, and the result is 1 rather than 1.01. Every naive decimal rounder in
   * every language has this edge. It is acceptable here because `roundTo` only
   * trims synthetic display values, never money and never anything summed.
   */
  it("rounds a value that is not exactly representable down, not half up", () => {
    expect(roundTo(1.005, 2)).toBe(1);
    // The same value one ulp higher does land on 1.01, which shows the cause
    // is the input's representation and not the rounding logic.
