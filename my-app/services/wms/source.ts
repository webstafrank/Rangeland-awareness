...
import { GIBS_LAYERS, KSA_LAYERS, layersForSource } from "@/services/wms/layers";
...
const KSA_GEOSERVER: WmsSource = {
  ...
  note: "No layers are registered for this source yet. Run `node scripts/wms-preflight.mjs` once GeoServer is up to list what it publishes, then add them to KSA_LAYERS in services/wms/layers.ts.",
};
 ...
 * app.
 *
 * Read on every call rather than memoised at module load. the services hold no mutable
