 ...
 *
 * No React, no Leaflet, no `window` anywhere under services/wms, for the same reason
 * as services/analysis: the interesting behaviour (source resolution, topic
 ...
 *
 * Structurally identical to `DateWindow` in services/run/config.ts and deliberately
 * not imported from it. services/wms has to stand on its own: the layer registry is
