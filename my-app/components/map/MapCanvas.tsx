...
export function MapCanvas({
  ...
  /*
   * The county outline, from the chrome tokens rather than a local hex.
   *
   * design/__tests__/tokens.test.ts asserts boundaryOnLight/boundaryOnDark
   * clear 3:1 against their grounds. Until this line, that assertion graded a
   * colour nothing painted: the boundaries were stroked with navy[700] on
   * dark and a loose "#dde5f0" on light, so the test passed while the map
   * shipped an unvalidated outline. Same class of bug as a mirror test that
   * reads the wrong block — green, and about the wrong thing.
   */
  const boundaryStroke =
    tone === "dark" ? chromeTokens.boundaryOnDark : chromeTokens.boundaryOnLight;
          ...
          <g>
            {context.map((area) => (
              <path
                ...
                d={area.path}
                fill={contextFill}
                stroke={showBoundaries ? boundaryStroke : "none"}
                strokeWidth={0.6}
              />
            ))}
          </g>
