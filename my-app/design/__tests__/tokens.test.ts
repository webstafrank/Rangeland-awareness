...
describe("globals.css mirrors design/tokens.ts", () => {
  ...
  it("leaves the focus ring to the app's chrome palette", () => {
    // This module used to require the focus ring to be --color-scarlet-mark,
    // back when the navy/scarlet system was the whole app rather than its
    // charts. It is not any more: lib/theme/palette.ts owns app chrome, its
    // focus ring is --color-accent, and palette.ts grades that ring against
    // every surface a focusable control can sit on.
    //
    // Asserted rather than deleted, and asserted in the negative, because the
    // failure this prevents is a real one: two modules each believing they
    // own the focus ring means whichever rule is written later silently wins,
    // and the graded ratio stops describing the ring that ships.
    const ring = /:focus-visible\s*\{[^}]*outline:[^;]*var\((--[a-z-]+)\)/.exec(css);
    expect(ring, ":focus-visible must set an outline from a token").not.toBeNull();
    expect(ring![1]).toBe("--color-accent");
  });
