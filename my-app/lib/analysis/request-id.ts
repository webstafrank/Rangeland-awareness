 ...
 * this id, and lib/handoff/areas.ts refuses to hand them back when the id in
 ...
 * come from lib/run/hash.ts, which is the one hash implementation in the app;
...
import { areaSeedKey, hashToId } from "@/lib/run/hash";
import type { AnalysisRequest } from "@/lib/analysis/request";

/**
 * The canonical string an id is derived from.
 *
 * Three rules, each mirroring canonicalConfigString in lib/run/hash.ts so the
