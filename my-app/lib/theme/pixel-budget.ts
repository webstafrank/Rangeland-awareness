 ...
 *
 * 5, and the number is forced rather than chosen. For a pixel's role to never
 * be ambiguous, no two tokens of DIFFERENT roles may have overlapping match
 * radii, so the tolerance must stay below HALF the smallest cross-role
 * distance in the shipped palette.
 *
 * A light palette is what forces it this low, and the reason is worth stating
 * because it is the opposite of the intuition. A dark theme separates its
 * tiers generously in sRGB — a midnight ground against a cobalt panel is 23
 * or more apart — so a tolerance of 10 was comfortable there. A light theme
 * crowds every surface into the top few percent of the cube: this app's page
 * (#f7f8fa) and its panel (#ffffff) are an obvious step apart to the eye and
 * only 11.75 apart numerically. Half of that is 5.87, so 10 would have given
 * the two radii a wide overlap, and every antialiased pixel along the edge of
 * every card would have been assigned to ground or structure by whichever was
 * marginally nearer. That is precisely the silent transfer between the 60 and
 * the 30 this constant exists to prevent.
 *
 * 5 sits under 5.87 with a little margin and still absorbs 8-bit rounding and
 * a ±2-per-channel blur (3.5). It does NOT absorb the ±5 blur (8.7) that the
 * old value did, and that is the deliberate cost: those pixels now fall
 * outside every radius and are counted as `unmatched` rather than being
 * assigned to whichever tier happens to be marginally nearer. Reporting them
 * as unknown is honest; splitting them on a coin flip would move percentage
 * points between the 60 and the 30 with nothing to show for it.
 *
 * `palette.test.ts` enforces the cross-role separation at 2x this value, so a
 * future token that breaks the invariant fails a test instead of quietly
 * skewing the budget.
 */
export const MATCH_TOLERANCE = 5;
