...
from apps.analysis.domain.ahp import (
    InvalidMatrix,
    assess,
    comparison_count,
...
def _find_fixtures() -> Path:
    """Walk up for contracts/ahp-fixtures.json rather than counting directories.

    A fixed `parents[n]` broke when this suite moved from
    ``services/analysis/tests/`` into the Django app, and a wrong hop count
    fails as "fixtures not found" at a path nobody recognises. Walking up is
    immune to where the suite lives, which matters for a file whose entire job
    is to be shared with the TypeScript side.
    """
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "contracts" / "ahp-fixtures.json"
        if candidate.is_file():
            return candidate
    return here.parents[-1] / "contracts" / "ahp-fixtures.json"
